...
import Sidebar from "./empSidear";
import { Camera, Upload, Trash2, Save, X } from 'lucide-react';
import Header from './empheader';
import ProfileService from '../../api/profile'; // Import the new API service
...
const HrProfilePage = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      try {
        // Use ProfileService to update profile image
  ...
  const deleteProfilePicture = async () => {
    // Note: Implement backend endpoint for removing profile picture if needed
    setProfileImage("/api/placeholder/200/200");
    // You might want to call a backend method to actually remove the image
  };

  const handleSaveChanges = () => {
    // Note: Implement a method in ProfileService to update full profile if needed
    ...
    <div className="flex bg-gray-50">
      ...
      <div className="flex-1 p-6">
        <div className="mx-auto">
          <div className={`flex-1 transition-all duration-300 p-6 ${isSidebarOpen ? "" : "ml-16"}`}>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="h-40 bg-gradient-to-r from-gray-500 to-gray-600 relative">
                ...
                <div className="absolute bottom-4 right-6">
                  ...
                  >
                    {isEditing ? 'Cancel' : 'Edit Profile Image'}
                  </button>
                </div>
              </div>
